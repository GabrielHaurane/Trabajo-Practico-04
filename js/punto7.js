...
let espacio = 0;
if (confirm(`desea modificar el espacio de su agenda`)) {
  espacio = Math.abs(
    parseInt(prompt(`indique la cantidad del espacio de su agenda`))
  );
} else {
  espacio = 10;
}
const agenda = {
  ...
  agregarContacto: function (contacto) {
    if (this.huecosLibres() > 0) {
      ...
      alert(`No hay espacio suficiente en la agenda`);
    }
  },
  existeContacto: function () {
    if (this.contactos.find((item) => item.nombre === contacto.nombre)) {
      alert(`el contacto que desea agregar ya existe`);
    } else {
